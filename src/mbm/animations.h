...
MBM_NO_ABI uint64_t animations_get_animation_duration (struct animations * self, int ianim);
...
MBM_NO_ABI void animations_update (const struct animations * self, int ianim, uint64_t anim_phase_shift, uint64_t tnow, uint64_t * t_frame_expires, int * iframe);
